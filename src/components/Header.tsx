...
function Header() {
  return (
    <header className="relative grid grid-cols-[1fr_auto_1fr] w-full items-center h-[72px] px-8 bg-[var(--azul-escuro)]/95 backdrop-blur-md border-b border-blue-500/15 overflow-hidden">
...
function NavItem({ to, children }: { to: string; children: React.ReactNode }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) => `relative px-3.5 py-1.5 rounded-md text-[11px] font-semibold tracking-widest uppercase font-[Rajdhani,sans-serif] transition-all duration-250 group
        ${isActive
          ? "text-blue-400"
          : "text-white hover:text-white hover:bg-blue-500/8"
        }
      `}
    >
      {({ isActive }) => (
        <>
          {children}
          <span className={`absolute bottom-0.5 left-3.5 right-3.5 h-[1.5px] rounded-full bg-blue-400 transition-transform duration-250 origin-left
